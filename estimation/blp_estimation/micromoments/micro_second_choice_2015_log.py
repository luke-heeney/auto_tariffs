...
def build_second_choice_moments_2015_log(
    ...
    """
    Variant of the 2015 second-choice micro-moments that works on standardized log
    characteristics instead of raw levels:
      - corr(log_size_std_first, log_size_std_second)
      - corr(log_hp_std_first, log_hp_std_second)
      - corr(ln_mpg_std_first, ln_mpg_std_second) (ICE+hybrid depending on flag)
      - type/brand moments are identical to the original version.
    """
    ...
    # Use standardized log variables
    size_map_2015 = dict(
        zip(
            prod2015_ids_str,
            pd.to_numeric(prod2015["log_size_std"], errors="coerce"),
        )
    )
    hp_map_2015 = dict(
        zip(
            prod2015_ids_str,
            pd.to_numeric(prod2015["log_hp_std"], errors="coerce"),
        )
    )

    # Use raw ln_mpg_ice (not standardized) for the MPG attribute
    mpg_col = "ln_mpg_ice"

    mpg_map_2015 = dict(
        zip(prod2015_ids_str, pd.to_numeric(prod2015[mpg_col], errors="coerce"))
    )
    price_map_2015 = dict(
    ...
    vt_vals_2015 = prod2015["vehicle_type"].fillna("").str.lower().str.strip()
    vt_map_2015 = dict(zip(prod2015_ids_str, vt_vals_2015))

    euro_vals_2015 = (
        pd.to_numeric(prod2015["euro_brand"], errors="coerce").fillna(0.0)
    )
    us_vals_2015 = pd.to_numeric(prod2015["us_brand"], errors="coerce").fillna(0.0)
    ...
    def _all_inside_pairs_weights(t, products, agents):
        ...
        # pad outside option as k=0 column
    ...
    def build_second_choice_dataset_2015_icehyb():
        def compute_weights(t, products, agents):
            ...
            for j in range(J):
                if is_non_ev[j]:
                    valid_k = np.where((is_non_ev == 1) & (np.arange(J) != j))[0]
                    if valid_k.size > 0:
                        W[j, valid_k] = 1.0 / valid_k.size

            W_full = np.zeros((J, J + 1), dtype=np.float64)
            W_full[:, 1:] = W
            return np.broadcast_to(W_full[None, :, :], (I, J, J + 1))
    ...
    def make_type_brand_second_choice_parts_2015(dataset):
        ...
        def _build_pair(name_prefix, vec_index):
            def _values_num(t, products, agents):
                v_van, v_truck, v_suv, v_euro, v_us, v_lux = _get_vectors(products)
        ...
        num_van_2015, den_van_2015 = _build_pair("E[1{j van} * 1{k van}]", 0)
        num_truck_2015, den_truck_2015 = _build_pair("E[1{j truck} * 1{k truck}]", 1)
        num_suv_2015, den_suv_2015 = _build_pair("E[1{j suv} * 1{k suv}]", 2)
        num_euro_2015, den_euro_2015 = _build_pair("E[1{j euro} * 1{k euro}]", 3)
        num_us_2015, den_us_2015 = _build_pair("E[1{j us} * 1{k us}]", 4)
        num_lux_2015, den_lux_2015 = _build_pair("E[1{j luxury} * 1{k luxury}]", 5)
    ...
    # Keep the original targets for now so weights stay comparable; they won't match the
    # standardized-log scale but are sufficient for diagnostics.
    TARGETS_2015 = {
        "size": 0.782,
        "hp": 0.674,
        "mpg": 0.611,
    ...
    # continuous attributes (using standardized logs)
    size_parts = make_second_choice_parts_2015(d2015, "size", "log_size_std")
    hp_parts = make_second_choice_parts_2015(d2015, "hp", "log_hp_std")
    mpg_parts = make_second_choice_parts_2015(d2015_icehyb, "mpg", "ln_mpg_std")
    price_parts = make_second_choice_parts_2015(d2015, "price", "price")
    ...
    if include_mpg == 1:
        mpg_moments_list = [
            pyblp.MicroMoment(
                "corr(log_mpg_std_first, log_mpg_std_second) | 2015 (ICE+hybrid only)",
                TARGETS_2015["mpg"],
                mpg_parts,
                _corr_value,
                _corr_grad,
    ...
    if include_hp == 1:
        hp_moments_list = [
            pyblp.MicroMoment(
                "corr(log_hp_std_first, log_hp_std_second) | 2015 (all vehicles)",
                TARGETS_2015["hp"],
                hp_parts,
                _corr_value,
                _corr_grad,
    ...
    if include_size == 1:
        size_moments_list = [
            pyblp.MicroMoment(
                "corr(log_size_std_first, log_size_std_second) | 2015 (all vehicles)",
                TARGETS_2015["size"],
                size_parts,
                _corr_value,
                _corr_grad,
    ...
    second_choice_moments_2015_log = [
        pyblp.MicroMoment(
    ...
    second_choice_moments_2015_log += mpg_moments_list
    second_choice_moments_2015_log += size_moments_list
    second_choice_moments_2015_log += hp_moments_list
    second_choice_moments_2015_log += eu_moments_list
    second_choice_moments_2015_log += us_moments_list

    return second_choice_moments_2015_log
